...
> **Normative principles:** the category's founding principles — the
...
- [ODAP data-room cover documents](https://github.com/open-data-agent-platform/awesome-odap/blob/main/reports/DATA_ROOM_COVER.md)
...
The category rests on two complementary artifacts:

- **The Five Musts** (principles) — *what any agent-to-data access must be*:
  Open, Semantic, Governed, Auditable, Cost-controlled.
  Defined in [The ODAP Manifesto](https://googlesql.com/odap/manifesto).
- **The Five Layers** (reference architecture) — *how a platform implements
  those principles in practice*, described below.
...
### How the Five Musts map to the Five Layers

| Principle (Must) | Implementing layer(s) |
|---|---|
| **Open** | Universal Data Gateway |
| **Semantic** | Smart Semantic Layer |
| **Governed** | Multi-Agent SQL Engine (validator) + Governance & Observability |
| **Auditable** | Governance & Observability (audit logs, lineage) |
| **Cost-controlled** | Multi-Agent SQL Engine (dry-run estimation) + Governance & Observability (budgets, circuit-breakers) |
| — | Persistent Memory *(architecture enhancement: improves accuracy over time; not an entry criterion)* |

A platform that satisfies all Five Musts qualifies as an ODAP. Persistent
Memory strengthens an implementation but is not required for category
membership.
...
- [Open Data Agent Platform (GoogleSQL.com)](https://googlesql.com/) -
...
**Governed**
- Does it enforce permissions and policies in the path of the query?
...
**Auditable**
- Does it keep audit logs of generated queries, approvals, and executions?
- Does it expose answer confidence, lineage, or validation evidence?

**Cost-controlled**
- Does it estimate or control query cost before execution?

**Architecture enhancement (optional)**
- Does it remember corrections and user feedback across sessions?
